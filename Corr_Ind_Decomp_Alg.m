...
delta = opts.delta; % threshold for minimum considerable variance (for stabilization)
Eps = sqrt(m)*1e-3; % approxiation accuracy threshold
rho = opts.rho; % penalty parameter

T_seq = round(linspace(1,opts.T,opts.Numiters)); % gradual increasing of sparsity (warm start)
...
for t = 1:opts.Numiters
    ...
    %                            {A,D} update
    if opts.DL == true
        [D1,D2,A1,A2] = SCDL(X1-E1,X2-E2,D1,D2,T_seq(t),Eps,opts.DLiters,1);
        
    else                        % A update only
        [A1,A2] = SOMP_C(X1-E1,X2-E2,D1,D2,T_seq(t),Eps);
    end
